...
public class FileScanImportExportRepositoryTest {
    ...
    public void importScans_shouldImportScansInFile() throws IOException {
        ...
        Files.write(Paths.get(fileName), "[{\"id\": \"myScan\", \"path\": \"/path/to/scan\"}, {\"id\": \"secondScan\", \"path\": \"/path/to/second/scan\"}]".getBytes());
        ...
        assertTrue(scans.contains(new Scan("myScan", "/path/to/scan")));
        assertTrue(scans.contains(new Scan("secondScan", "/path/to/second/scan")));
    }
    ...
    public void exportScans_shouldExportScansToFile() throws IOException {
        // Arrange
        String fileName = randomTempFile();
        scanFacade.addScans(Lst(new Scan("scan", "/scan/path"), new Scan("anotherScan", "/second/path")));
        ...
        assertEquals("[{\"id\":\"scan\",\"path\":\"/scan/path\"},{\"id\":\"anotherScan\",\"path\":\"/second/path\"}]", result);
    }
    ...
    public void exportImport_endToEnd() throws IOException {
        // Arrange
        String fileName = randomTempFile();
        List<Scan> scans = Lst(new Scan("scan", "/scan/path"), new Scan("anotherScan", "/second/path"));
